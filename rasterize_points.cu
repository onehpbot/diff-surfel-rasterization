...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& cut_normals,
	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		colors2.contiguous().data<float>(),
		cut_normals.contiguous().data<float>(), 
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dcolors2 = torch::zeros({P, NUM_CHANNELS}, means3D.options());
  torch::Tensor dL_dcut_normals = torch::zeros({P, 2}, means3D.options());
  ...
  {  
	  ...
	  dL_dcolors2.contiguous().data<float>(),
	  dL_dcut_normals.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dcolors2, dL_dcut_normals, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
